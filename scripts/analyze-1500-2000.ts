...
// Parse CSV data for games ranked 1500-2000
function parseCSV(csvContent: string): BGGRankingData[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (values.length >= 8) {
      ...
      // Only include base games (not expansions) with valid ranks between 1500-2000
      if (game.is_expansion === '0' && game.rank && game.rank > 1500 && game.rank <= 2000) {
        games.push(game);
      }
    }
  }
...
// Analyze games ranked 1500-2000
async function analyze1500to2000Games() {
  console.log('🎯 Analyzing BGG ranked games 1500-2000...');
  ...
  try {
    ...
    // Parse CSV data for games 1500-2000
    const allGames = parseCSV(csvContent);
    console.log(`📋 Parsed ${allGames.length} base games from CSV (rank 1500-2000)`);
    
    if (allGames.length === 0) {
      console.log('❌ No games found in the 1500-2000 rank range. Check your CSV data.');
      return;
    }
    ...
    console.log(`\n📊 Games 1500-2000 Analysis Results:`);
    console.log(`🏆 Total games in range 1500-2000: ${allGames.length}`);
    ...
    if (missingGames.length === 0) {
      console.log('\n🎉 Congratulations! You have all games ranked 1500-2000 in your database!');
      return;
    }
    
    // Show breakdown by rank ranges within 1500-2000
    console.log(`\n📊 Missing games by rank range:`);
    const rankRanges = [
      { name: '1501-1600', min: 1501, max: 1600 },
      { name: '1601-1700', min: 1601, max: 1700 },
      { name: '1701-1800', min: 1701, max: 1800 },
      { name: '1801-1900', min: 1801, max: 1900 },
      { name: '1901-2000', min: 1901, max: 2000 }
    ];
    ...
    const missingGamesData = {
      ...
      rank_range: '1500-2000',
      source: 'BGG ranked games 1500-2000 (base games only)',
    ...
    fs.writeFileSync('missing-games-1500-2000.json', JSON.stringify(missingGamesData, null, 2));
    
    console.log(`\n💾 Saved detailed missing games data to missing-games-1500-2000.json`);
    console.log(`🎯 Missing games prioritized by BGG rank (best games in range first)`);
    
    // Show top 20 missing games in this range
    console.log(`\n🏆 Top 20 highest-ranked missing games (1500-2000):`);
    ...
    if (existingGames.length > 0) {
      console.log(`\n✅ Sample of games you already have in 1500-2000 range:`);
    ...
    console.log(`\n🚀 Next steps:`);
    console.log(`1. Review the missing-games-1500-2000.json file`);
    console.log(`2. Run import script if you want to add these ${missingGames.length} games to your database`);
    console.log(`3. These games ranked 1500-2000 are still quality games worth having!`);
    
  } catch (error) {
    console.error('❌ Error analyzing games 1500-2000:', error);
    throw error;
  }
}
...
if (require.main === module) {
  analyze1500to2000Games()
    .then(() => {
      console.log('\n✅ Games 1500-2000 analysis completed');
      process.exit(0);
    })
    .catch((error) => {
